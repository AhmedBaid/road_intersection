use macroquad::prelude::*;
#[derive(Clone)]
pub struct Car {
    pub direction: String,
    pub width: i32,
    pub height: i32,
    pub cord: (f32, f32),
    pub color: Color,
    pub root: String,
    pub speed: f32,
}

impl Car {
    pub fn new(direction: String, width: i32, height: i32, cord: (f32, f32), color: i32) -> Self {
        let color = match color {
            1 => YELLOW,
            2 => BLUE,
            _ => RED,
        };
        let root = Self::get_root(color, direction.clone());
        Self {
            direction,
            width,
            height,
            cord,
            color,
            root,
            speed: 160.0,
        }
    }
    pub fn get_root(color: Color, direction: String) -> String {
        match direction.as_str() {
            "up" => if color == YELLOW {
                return "right".to_string();
            } else if color == RED {
                return "left".to_string();
            }
            "down" => if color == YELLOW {
                return "up".to_string();
            } else if color == RED {
                return "right".to_string();
            }
            "left" => if color == YELLOW {
                return "up".to_string();
            } else if color == RED {
                return "down".to_string();
            }
            "right" => if color == YELLOW {
                return "down".to_string();
            } else if color == RED {
                return "up".to_string();
            }
            _ => {}
        }
        "".to_string()
    }

    pub fn update(&mut self, dt: f32) {
        let (mut x, mut y) = self.cord;

        match self.direction.as_str() {
            "up" => {
                if
                    y >= screen_height() / 2.0 + 12.0 &&
                    y <= screen_height() / 2.0 + 18.0 &&
                    self.color == RED
                {
                    y = screen_height() / 2.0 + 15.0;
                    x += self.speed * dt;
                } else if
                    y >= screen_height() / 2.0 - 48.0 &&
                    y <= screen_height() / 2.0 - 43.0 &&
                    self.color == YELLOW
                {
                    y = screen_height() / 2.0 - 45.0;
                    x -= self.speed * dt;
                } else {
                    y -= self.speed * dt;
                }
            }
            "down" => {
                if
                    y >= screen_height() / 2.0 - 48.0 &&
                    y <= screen_height() / 2.0 - 43.0 &&
                    self.color == RED
                {
                    y = screen_height() / 2.0 - 45.0;
                    x -= self.speed * dt;
                } else if
                    y >= screen_height() / 2.0 + 12.0 &&
                    y <= screen_height() / 2.0 + 18.0 &&
                    self.color == YELLOW
                {
                    y = screen_height() / 2.0 + 15.0;
                    x += self.speed * dt;
                } else {
                    y += self.speed * dt;
                }
            }
            "left" => {
                if
                    x >= screen_width() / 2.0 + 12.0 &&
                    x <= screen_width() / 2.0 + 18.0 &&
                    self.color == RED
                {
                    x = screen_width() / 2.0 + 15.0;
                    y -= self.speed * dt;
                } else if
                    x >= screen_width() / 2.0 - 48.0 &&
                    x <= screen_width() / 2.0 - 42.0 &&
                    self.color == YELLOW
                {
                    x = screen_width() / 2.0 - 45.0;
                    y += self.speed * dt;
                } else {
                    x -= self.speed * dt;
                }
            }
            "right" => {
                if
                    x >= screen_width() / 2.0 - 48.0 &&
                    x <= screen_width() / 2.0 - 42.0 &&
                    self.color == RED
                {
                    x = screen_width() / 2.0 - 45.0;
                    y += self.speed * dt;
                } else if
                    x >= screen_width() / 2.0 + 12.0 &&
                    x <= screen_width() / 2.0 + 18.0 &&
                    self.color == YELLOW
                {
                    x = screen_width() / 2.0 + 15.0;
                    y -= self.speed * dt;
                } else {
                    x += self.speed * dt;
                }
            }
            _ => {}
        }

        self.cord = (x, y);
    }
}
